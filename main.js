...
async function loadJSON(e) {
    ...
    try {
        ...
        for (const ent of entidades) {
            ...
            } else if (ent.type === "VuStaticModelEntity") {
                await crearObjetoDesdeEntidad(ent);
            }
        }
...
async function chargeModel(VuEngine){
    ...
    try{
        ...
        alert(`sorry the: ${VuEngine}`,err.message);
        return null;
    }
} 
...
async function crearObjetoDesdeEntidad(entidad) {
    const rutaModelo = entidad.modelAsset || entidad.ModelAsset;
    ...
    // APLICAMOS LA MISMA CONVERSIÓN DE EJES QUE LOS WAYPOINTS
    const pos = readVector(entidad);
    const rot = readVector(entidad.Rotation || entidad.rotation);

    modelo.position.set(
        pos.x,
        pos.z,       // Si ya tienes el ajuste global, quita el +0.8 aquí
        -pos.y
    );
    ...
    modelo.scale.copy(readVector(entidad.Scale || entidad.scale, new THREE.Vector3(1,1,1)));
